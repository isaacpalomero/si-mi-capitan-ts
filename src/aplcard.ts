...
export class APLHomeCardRequestInterceptor implements RequestInterceptor {
    public process(handlerInput: HandlerInput) {
        ...
        function withSimpleAPLCard(cardTitle: string, cardContent: string) {
        ...
        function withStandardAPLCard(cardTitle: string, cardContent: string, smallImageUrl?: string, largeImageUrl?: string) {
            if (supportsAPL(handlerInput)) {
                handlerInput.responseBuilder.addDirective({
                    type: "Alexa.Presentation.APL.RenderDocument",
                    document: APLDoc,
...
function supportsAPL(handlerInput: HandlerInput): boolean {
    if (handlerInput.requestEnvelope.context.System.device !== undefined) {
        return false;
    }
...
/*function deviceType(handlerInput: HandlerInput) {
    if (supportsAPL(handlerInput)) {
